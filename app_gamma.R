...
compute_curves_by_gamma <- function(p, sigma_pl, lambda, epsilon, gamma_grid) {
  H_p <- H(p)
...
ui <- fluidPage(
  titlePanel("Outcomes vs. γ (Risk Aversion)"),
  sidebarLayout(
    sidebarPanel(
      sliderInput("p", "Prior p (prob PL)", 0.01, 0.99, 0.5, 0.01),
      sliderInput("sigma_pl", "σ_PL (PL quality variance)", 0.1, 5, 2, 0.1),
...
server <- function(input, output, session) {
  gamma_grid <- seq(0, 1, length.out = 100)
  
  curves <- reactive({
    compute_curves_by_gamma(
      p = input$p,
  ...
  output$utilPlot <- renderPlot({
    ggplot(curves(), aes(x = gamma, y = value, color = type)) +
      geom_line(linewidth = 1) +
      labs(title = "Expected Utility vs γ", x = "γ (Risk Aversion)", y = "Expected Utility") +
      theme_minimal() + theme(legend.position = "bottom")
  })
  
  output$buyPlot <- renderPlot({
    ggplot(curves(), aes(x = gamma, y = buy, color = type)) +
      geom_line(linewidth = 1) +
      labs(title = "Buying Probability vs γ", x = "γ (Risk Aversion)", y = "P(buy)") +
      theme_minimal() + theme(legend.position = "bottom")
  })
  
  output$discrimPlot <- renderPlot({
    ggplot(curves(), aes(x = gamma, y = discrim, color = type)) +
      geom_line(linewidth = 1) +
      labs(title = "Discrimination vs γ", x = "γ (Risk Aversion)", y = "P(buy|CZ) - P(buy|PL)") +
      theme_minimal() + theme(legend.position = "bottom")
  })
  
  output$infoPlot <- renderPlot({
    ggplot(curves(), aes(x = gamma, y = info, color = type)) +
      geom_line(linewidth = 1) +
      labs(title = "Information Acquired vs γ", x = "γ (Risk Aversion)", y = "Mutual Information") +
      theme_minimal() + theme(legend.position = "bottom")
  })
}
